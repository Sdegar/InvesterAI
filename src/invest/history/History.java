...
public abstract class History {
   ...
   public void addSession(Session session) {
      String SQL = "INSERT INTO " + id +
            " (date, openPrice, closePrice, intraMax, intraMin) " +
            "values (?, ?, ?, ?, ? ,?)";
   ...
   public Session getSession(Date date) {
      ...
      Session session = jdbcTemplateObject.queryForObject(SQL, 
                        new Object[]{sqlDate}, mapper);
      return session;
   }
   
   public List<Session> getSessionsTo(int index, int count) {
      String SQL = "SELECT * FROM " + id + " WHERE id <= ? LIMIT ?";
      List<Session> sessions = jdbcTemplateObject.query(SQL, 
                        new Object[]{index, count}, mapper);
      return sessions;
   }
   ...
   public List<Session> getSessionsTo(Date date, int count) {
      ...
      String SQL = "SELECT * FROM " + id + " WHERE date <= ?";
      List<Session> sessions = jdbcTemplateObject.query(SQL, 
                        new Object[]{sqlDate, count}, mapper);
      return sessions;
   }
