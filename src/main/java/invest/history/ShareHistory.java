...
public class ShareHistory extends History {
   ...
   public void addSession(Session session) {
      String SQL = "INSERT INTO " + id + " (date, openPrice, closePrice, intraMax, intraMin, per) "
            + "values (?, ?, ?, ?, ? ,?)";
      java.sql.Date sqlDate = new java.sql.Date(session.getDate().getTime());
      double per = ((ShareSession) session).getPER();
      jdbcTemplateObject.update(SQL, sqlDate, session.getOpeningPrice(), session.getClosingPrice(),
   ...
   public void create() {
      String SQL = "CREATE TABLE " + id + " ( " + "id INTEGER UNSIGNED AUTO_INCREMENT," + "date DATETIME DEFAULT NULL,"
            + "openPrice FLOAT," + "closePrice FLOAT," + "intraMax FLOAT," + "intraMin FLOAT," + "per FLOAT,"
            + "PRIMARY KEY (id)" + ");";
